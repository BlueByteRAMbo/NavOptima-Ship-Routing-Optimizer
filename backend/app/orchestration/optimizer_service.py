...
def handle_simulation_event(event: SimulationEvent) -> SimulationResponse:
    ...
    3. Checks spatial relevance against remaining route nodes and current ship position.
    ...
    # Measure min distance from event center to ship's current position and remaining nodes
    min_dist_to_ship_km = _haversine_km(event.lat, event.lon, curr_lat, curr_lon)
    min_dist_to_remaining_km = min(
        [min_dist_to_ship_km] + [_haversine_km(event.lat, event.lon, n.latitude, n.longitude) for n in remaining_nodes],
        default=9999.0
    )

    # Measure min distance to ALL nodes (to detect if event is behind ship)
    all_nodes = [graph.get_node(nid) for nid in all_path_nodes if graph.get_node(nid)]
    min_dist_to_all_km = min(
        [_haversine_km(event.lat, event.lon, n.latitude, n.longitude) for n in all_nodes],
        default=9999.0
    )
    ...
    # 3. Handle Spatially Relevant Event (Port Congestion, Storm, or Security)
    ...
    # Storm or Security Threat Handling
    speed_k = 8.0 if event.type == "storm" else 0.0
    dir_deg = 90.0 if event.type == "storm" else 0.0
